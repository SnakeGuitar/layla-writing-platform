...
export const ChapterVersionModel = model(
  "ChapterVersion",
...
router.put(
  "/manuscripts/:projectId/:manuscriptId/chapters/:chapterId/autosave",
  async (req, res) => {
...
- **Control de Versiones**: Un panel lateral (`SidebarVersionHistory`) dibujará un _Timeline_ leyendo de `/api/chapters/:chapterId/versions` y usará una librería de Diff de C# para mostrar rojo/verde en las diferencias del `RichTextBox`.
